...
/// convertWithCVMat
/// - Parameters:
///   - originalCVMat: originalCVMat
///   - outputCVMat: outputCVMat
- (void)convertWithCVMat: (cv::Mat &)originalCVMat output:(cv::Mat &)outputCVMat {
    cv::Mat& rawBgraImage = originalCVMat;
...
//     1. cropt
    ...
    rgb_image.convertTo(outputCVMat, CV_32FC3);
}

/// inferWithImageBuffer
/// - Parameters:
...
- (NSArray<NSValue *> *)inferWithImageBuffer: (CMSampleBufferRef)sampleBuffer {
    @try {
        ...
        cv::Mat input = cv::Mat((int)bufferHeight, (int)bufferWidth, CV_8UC4, pixel);
        //End processing
        CVPixelBufferUnlockBaseAddress( pixelBuffer, 0 );
        cv::Mat output;
        [self convertWithCVMat:input output:output];
        return [self inferOriginal:input resized:output];
...
/// inferWithImage
/// - Parameter inputImage: inputImage
- (NSArray<NSValue *> *)inferWithImage:(UIImage *)inputImage {
    @try {
        cv::Mat input = [OpenCVUtil cvMatFromUIImage:inputImage];
        cv::Mat output;
        [self convertWithCVMat:input output:output];
        return [self inferOriginal:input resized:output];
    } @catch (NSException *exception) {
...
///   - original: cv::Mat
///   - resized: cv::Mat
- (NSArray<NSValue *> *)inferOriginal: (cv::Mat)original resized: (cv::Mat)resized {
    assert(resized.rows == wanted_input_height);
    assert(resized.cols == wanted_input_width);
    assert(resized.channels() == wanted_input_channels);
    assert(resized.type() == CV_32FC3);
    
    cv::Mat input_rgb;
    resized.convertTo(input_rgb, CV_8UC3);
    self.current_image = input_rgb;
    ...
    cv::Mat input_dummy = resized.clone();
    ...
    if (validate_coords(X_coords, Y_coords)) {
        ...
        return @[
            [NSValue valueWithCGPoint: CGPointMake(X_coords[0] * original_width / wanted_input_width, Y_coords[0] * original_height / wanted_input_height)],
            [NSValue valueWithCGPoint: CGPointMake(X_coords[1] * original_width / wanted_input_width, Y_coords[1] * original_height / wanted_input_height)],
            [NSValue valueWithCGPoint: CGPointMake(X_coords[2] * original_width / wanted_input_width, Y_coords[2] * original_height / wanted_input_height)],
            [NSValue valueWithCGPoint: CGPointMake(X_coords[3] * original_width / wanted_input_width, Y_coords[3] * original_height / wanted_input_height)]
        ];
    } else {
        // there is no rectangle.
        return  @[];
...
/// capture
/// - Parameter complation: ComplationBlock
- (void)capture:(ComplationBlock)complation {
    @try {
        if (self.current_image.empty() == YES || self.keypoints.empty() == YES) {
            complation(nil, nil, nil);
            return;
        }
        cv::Mat inputImage = self.current_image;
        auto coord = self.keypoints;
...
//        std::vector< std::vector<double> > new_coord {coord1,coord2,coord3,coord4};
        ...
        cv::Mat resultMat = cropped_img.clone();
        UIImage *origin = [OpenCVUtil UIImageFromCVMat:inputImage];
        UIImage *croped = [OpenCVUtil UIImageFromCVMat:resultMat];
        
        int original_height, original_width;
        original_height = inputImage.rows;
        original_width = inputImage.cols;
        
        // [0: topLeft, 1: topRight, 2: bottomLeft: 3: bottomRight]
        auto keypoint = self.keypoints;
        ...
        auto br = keypoint[3];
        NSArray *points = @[
            [NSValue valueWithCGPoint: CGPointMake(tf[0] * original_width / wanted_input_width, tf[1] * original_height / wanted_input_height)],
            [NSValue valueWithCGPoint: CGPointMake(tr[0] * original_width / wanted_input_width, tr[1] * original_height / wanted_input_height)],
            [NSValue valueWithCGPoint: CGPointMake(bl[0] * original_width / wanted_input_width, bl[1] * original_height / wanted_input_height)],
            [NSValue valueWithCGPoint: CGPointMake(br[0] * original_width / wanted_input_width, br[1] * original_height / wanted_input_height)]
        ];
        complation(origin, croped, points);
    } @catch (NSException *exception) {
        complation(nil, nil, nil);
    } @finally {
